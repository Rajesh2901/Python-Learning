...
export class ApiService {
  ...
  private fallbackQuestions: InterviewQuestion[] = [
    ...
    },
    {
      ...
      problem_statement: 'Given the root of a binary tree, return its maximum depth.\n\nA binary tree depth is the number of nodes along the longest path from root to farthest leaf node.',
  ...
  private fallbackChallenges: ChallengeProblem[] = [
    ...
    {
      id: 2,
      title: '2D Vector with Dunder Methods',
      difficulty: 'Medium',
      category: 'Python Data Model',
      description: 'Build a Vector2D class that implements `__add__`, `__repr__`, and `__eq__` dunder methods.',
      starter_code: `class Vector2D:\n    def __init__(self, x, y):\n        self.x = x\n        self.y = y\n\n    # Implement __add__, __repr__, and __eq__\n    pass\n\nv1 = Vector2D(2, 4)\nv2 = Vector2D(3, 1)\nv3 = v1 + v2\nprint("Result Vector:", v3)  # Expected: Vector2D(5, 5)`,
      solution_code: `class Vector2D:\n    def __init__(self, x, y):\n        self.x = x\n        self.y = y\n\n    def __add__(self, other):\n        return Vector2D(self.x + other.x, self.y + other.y)\n\n    def __repr__(self):\n        return f"Vector2D({self.x}, {self.y})"\n\n    def __eq__(self, other):\n        return self.x == other.x and self.y == other.y\n\nv1 = Vector2D(2, 4)\nv2 = Vector2D(3, 1)\nprint("Result Vector:", v1 + v2)`,
      reference_url: 'https://docs.python.org/3/reference/datamodel.html'
    },
    {
      id: 3,
    ...
    },
    {
      ...
      description: 'Use `asyncio.gather()` to fetch three simulated API responses concurrently.',
      ...
      reference_url: 'https://docs.python.org/3/library/asyncio.html'
    }
  ];
  ...
  private fallbackCodeReviews: CodeReviewSimulation[] = [
    ...
    {
      id: 3,
      title: 'Unintended Global Mutation',
      category: 'Scope & Closures',
      code_with_bugs: `counter = 0\n\ndef increment():\n    counter = counter + 1\n    return counter\n\n# UnboundLocalError: local variable 'counter' referenced before assignment\nincrement()`,
      bug_line_number: 4,
      explanation: 'Assigning to a variable inside a function makes Python treat it as a local variable. To modify an outer global counter, you must explicitly declare `global counter` or use a closure with `nonlocal`.',
      corrected_code: `counter = 0\n\ndef increment():\n    global counter\n    counter += 1\n    return counter`
    }
  ];
  ...
  runCode(code: string): Observable<CodeRunResponse> {
    return this.http.post<CodeRunResponse>(`${this.baseUrl}/run`, { code }).pipe(
      catchError(() => {
        // Safe simulated fallback output for static deployment
        return of({
  ...
  getRecommendations(): Observable<InterviewQuestion[]> {
    return this.http.get<InterviewQuestion[]>(`${this.baseUrl}/recommendations`).pipe(
      catchError(() => of(this.fallbackQuestions.slice(0, 2)))
    );
  }
